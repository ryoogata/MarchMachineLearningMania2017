...
result.xglinear.df <- readRDS("result/result.xglinear.df.data")
...
# 結果の保存
result.xglinear.df <- rbind(result.xglinear.df, summaryResult(model_list[[1]]))
saveRDS(result.xglinear.df, "result/result.xglinear.df.data")

# predict() を利用した検算 
if (is.null(model_list[[1]]$preProcess)){
  # preProcess を指定していない場合
  pred_test.verification <- predict(
    fit.model_list[[1]]$finalModel
...
log_loss(TRAIN.TEST$flg, 1 - pred_test.verification)
...
if (is.null(model_list[[1]]$preProcess)){
  ...
  pred_test  <- predict(
    model_list[[1]]$finalModel
    ,as.matrix(subset(TEST, select = -c(id)))
  )
  pred_test <- 1 - pred_test.xgbLinear
  PREPROCESS <- "no_preProcess"
} else {
  # preProcess を指定している場合
  pred_test  <- preProcess(
    subset(TEST, select = -c(id))
    ,method = my_preProcess
  ) %>%
    predict(., subset(TEST, select = -c(id))) %>%
    as.matrix(.) %>%
    predict(model_list[[1]]$finalModel, .)
  pred_test <- 1 - pred_test.xgbLinear
...
for(NUM in 1:10){
  ...
  SUBMIT_FILENAME <- paste("./submit/submit_", DATE, "_", NUM, "_", PREPROCESS, "_xglinear.csv", sep = "")
